...
export default function ResultsMexicanoPage() {
  const state = useMemo(() => {
    const s = loadState();
  ...
  const view = useMemo(() => {
    ...
    const rows = isTeam
      ? sortTeams(state.teams, state.rounds).map((t) => ({ id: t.id, name: t.name, games: t.games, pf: t.pointsFor, pa: t.pointsAgainst }))
      : sortPlayers(state.players, state.rounds).map((p) => ({ id: p.id, name: p.name, games: p.games, pf: p.pointsFor, pa: p.pointsAgainst }));
  ...
  if (!state || !view) {
    return (
      <div className="p-6">
        ...
        <Link className="text-emerald-700 underline" href="/mexicano">Wróć</Link>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link className="text-emerald-700 underline" href="/mexicano">Wróć do turnieju</Link>
      </div>
